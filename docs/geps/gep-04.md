...
```{note}
Added 2026-05-19 as part of the runtime type-checking rollout — see {ref}`gep-9`.
```
...
keeps policy code readable for authors who think in terms of single households, keeps
the function body amenable to static type checking with `ty`, and lets the vectorisation
layer remain implementation detail. Parameters that are themselves scalars at the DAG
level (e.g., the output of a `@param_function`) carry the same scalar annotations; the
validator distinguishes column from parameter inputs via the dependency graph, not via
...
underlying legal text — one household at a time — and let `ty` check the body against
the same scalar contract the author had in mind. The auto-vectorised wrapper that the
DAG actually calls is generated separately; see {ref}`gep-9` for the
wrapper-annotation-strip pattern that prevents the wrapper from inheriting the user's
scalar annotations and surfacing them as false positives under the runtime claw.

#### Migration

Existing GETTSIM policy functions written in scalar mode (the overwhelming majority)
need no changes — scalar annotations were already the convention. Existing functions
declared with `vectorization_strategy="not_required"` need to be reviewed: their column
parameters should carry column annotations, and any remaining scalar annotations on
column inputs are now errors at import time.
